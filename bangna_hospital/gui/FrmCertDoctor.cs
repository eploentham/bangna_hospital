...
namespace bangna_hospital.gui
{
    public partial class FrmCertDoctor : Form
    {
        ...
        ,"มะเร็งต่อมน้ำเหลือง","ตรวจหัวใจ วิ่งสายพาน","เกล็ดเลือดต่ำ","โรคหยุดหายใจขณะหลับระดับความรุนแรงมาก(AHI 48.8, low sat 89%)"};
        ...
        ,"ตรวจ , จ่ายยา ", "จำเป็นต้องรักษาด้วยเครื่องอัดอากาศแรงดันบวก"};
        ...
        private void initConfig()
        {
            ...
            AN = "";
            DataTable dtan = bc.bcDB.pt08DB.SelectANByHN(HN, VSDATE, PRENO);
            if (dtan != null && dtan.Rows.Count > 0)
            {
                AN = dtan.Rows[0]["MNC_AN_NO"].ToString()+"."+dtan.Rows[0]["MNC_AN_YR"].ToString();
            }
            ptt = new Patient();
        ...
        private void BtnPrintCert_Click(object sender, EventArgs e)
        {
            ...
            if (lbDtrName.Text.Length > 0)
            {
                ...
                {
                    printCertDoctoriTextSharpThaiPhysical();        //กายภาพ
                }
                else
                {
