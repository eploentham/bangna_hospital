...
{
	public class CellNoteManager1
	{
		//------------------------------------------------------------------------------------------------------
		#region ** fields

		private C1FlexGrid _flex;               // grid that owns the notes
		private CellNote1 _note;             // note currently displayed
		...
		public CellNoteManager1(C1FlexGrid flex)
		{
		...
		private CellNote1 GetNoteAtMouse()
		{
			...
			if (row >= _flex.Rows.Fixed && col >= _flex.Cols.Fixed)
			{
				...
				return rg.UserData as CellNote1;
			}
			return null;
		}
