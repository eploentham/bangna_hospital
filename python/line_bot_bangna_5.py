...
def sendLab(hn, reqdate2, userid):
    ...
    for reshe in myresult:
        if reshe[0] == 'HE001':
            ...
            draw.text((x,y+30), labname, font=fnt36, fill=(0,0,0))
            ...
            draw.text((x+col5,y+30), normal, font=fntS28, fill=(0,0,0))  # column ค่ามาตรฐาน
            draw.text((x+col6,y+30), unit, font=fntS28, fill=(0,0,0))      # column unit name
            y = y+30
            row = row+1
    if statusHE001 == '1':
    ...
    for reshe in myresult:
        if reshe[0] == 'MS001':
            ...
            draw.text((x+col3,y+30), labval, font=fnt36, fill=(0,0,0))
            draw.text((x+col4,y+30), interpret, font=fnt36, fill=(0,0,0))
            draw.text((x+col5,y+30), normal, font=fntS28, fill=(0,0,0))
            draw.text((x+col6,y+30), unit, font=fntS28, fill=(0,0,0))
            y = y+30
            row = row+1
    if statusMS001 == '1':
        draw.text((10,20), "โรงพยาบาล บางนา5", font=fnt45, fill=(0,0,0))
        draw.text((300,20), "Result LAB UA ", font=fnt45, fill=(0,0,0))
        draw.text((520,20), "patient name "+pttname+" hn "+hn+" reqest date "+reqdate, font=fnt, fill=(0,0,0))
        draw.text((1300,740), "send date xx/xx/yyyy", font=fnt, fill=(0,0,0))
        image.save(filename)
        time.sleep(1)
        img = cv2.imread(filename, cv2.IMREAD_UNCHANGED)
    ...
    for res in myresult:
        ...
        #x = x + 10
    draw.text((10,20), "โรงพยาบาล บางนา5", font=fnt45, fill=(0,0,0))
    draw.text((300,20), "Result LAB ", font=fnt45, fill=(0,0,0))
    draw.text((520,20), "patient name "+pttname+" hn "+hn+" reqest date "+reqdate, font=fnt, fill=(0,0,0))
    draw.text((1300,740), "send date xx/xx/yyyy", font=fnt, fill=(0,0,0))
    image.save(filename)
    time.sleep(1)
